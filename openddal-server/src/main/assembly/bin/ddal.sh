...
if [ "x$DDAL_HOME" = "x" ]; then
    DDAL_HOME="`dirname "$0"`/.."
fi

# The directory where openddal's configs live (required)
if [ "x$DDAL_CONF" = "x" ]; then
    DDAL_CONF="$DDAL_HOME/conf"
fi

# The directory where openddal's bin live (required)
if [ "x$DDAL_BIN" = "x" ]; then
    DDAL_BIN="$DDAL_HOME/bin"
fi

# The directory where openddal's bin live (required)
if [ "x$DDAL_LOG" = "x" ]; then
    DDAL_LOG="$DDAL_HOME/logs"
fi

# This can be the path to a jar file, or a directory containing the 
# compiled classes. NOTE: This isn't needed by the startup script,
# it's just used here in constructing the classpath.
DDAL_BIN="$DDAL_HOME/build/classes/main"
