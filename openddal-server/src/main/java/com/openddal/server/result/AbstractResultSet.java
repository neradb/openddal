...
import com.openddal.result.SimpleRowSource;
...
/**
 * <p>
 * This class is an abstract result set and meta data implementation.
 ...
 * 
 * <p>
 * This class provides a template for simplifying ResultSet and ResultSetMetaData 
 * implementations. The subclass instance of this can be simple such as 
 * SimpleResultSet instances, also could be reusable such as ThreadLocalResultSet
 * instances.
 * </p>
 * 
 * @author little-pan
...
public abstract class AbstractResultSet implements ResultSet, ResultSetMetaData{
	...
	protected SimpleRowSource source;
     ...
     */
	protected AbstractResultSet(final SimpleRowSource source) {
        this.source = source;
    }
    ...
    public boolean next() throws SQLException {
        if (source != null) {
            if(iterator().nextFromSource()){
            	return true;
            }
        } else if (rows != null) {
            if(iterator().nextFromRows()){
            	return true;
            }
        }
        if (autoClose) {
    ...
    /**
     * Reseting access status.
     */
    protected ResultSet reset()throws SQLException{
    	iterator().reset();
    	if (source != null) {
            source.reset();
        }
 ...
 // --- private -----------------------------
    ...
    protected void update(int columnIndex, Object obj) throws SQLException {
        checkColumnIndex(columnIndex);
        iterator().update(columnIndex, obj);
    }

    protected void update(String columnLabel, Object obj) throws SQLException {
        update(findColumn(columnLabel), obj);
    }
	...
	protected Object get(int column) throws SQLException {
		return (iterator().get(column));
	}
    
    protected abstract ResultSetIterator iterator();
   ...
   protected static class SimpleResultSetIterator implements ResultSetIterator{
	    ...
	    public SimpleResultSetIterator(){
	    	
	    }
	    
		@Override
		public boolean nextFromSource() throws SQLException {
			rowId++;
           currentRow = resultSet.source.readRow();
           if (currentRow != null) {
               return true;
           }
			return false;
		...
		@Override
		public boolean nextFromRows() {
			final List<Object[]> rows = resultSet.rows;
		...
		public Object get(int column) throws SQLException {
			...
			resultSet.checkColumnIndex(column);
			--column;
			Object o = column < currentRow.length ?
		...
		@Override
		public void update(final int column, final Object value) {
		...
		@Override
		public void setResultSet(final AbstractResultSet resultSet){
