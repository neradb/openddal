...
public final class ServerParseShow {
    ...
    public static final int CHARSET = 4;
    public static final int COLLATION = 5;
    public static final int CONNECTION = 6;
    public static final int VARIABLES = 7;
    public static final int SESSION_VARIABLES = 8;
    public static final int SESSION_STATUS = 9;
    public static final int SLOW = 10;
    public static final int PHYSICAL_SLOW = 11;
    // show engines
    // @author little-pan
    // @since 2016-07-13
    public static final int ENGINES = 12;
    public static final int FULL_TABKES = 13;
    ...
    public static int parse(String stmt, int offset) {
        ...
        for (; i < stmt.length(); i++) {
            switch (stmt.charAt(i)) {
                ...
                case 'C':
                case 'c':
                    int type = connectionCheck(stmt, i);
                    type = type == OTHER ? showCharset(stmt, i) : type;
                    return type == OTHER ? showCollation(stmt, i) : type;
                case 'V':
                case 'v':
    ...
    static final int engineCheck(final String stmt, final int offset) {
        if (stmt.length() > offset + "NGINES".length()) {
            ...
            if ((c1 == 'N' || c1 == 'n') && (c2 == 'G' || c2 == 'g') && (c3 == 'I' || c3 == 'i')
                    && (c4 == 'N' || c4 == 'n') && (c5 == 'E' || c5 == 'e') && (c6 == 'S' || c6 == 's')
    ...
    // SHOW CHARSET
    static int showCharset(String stmt, int offset) {
    ...
    public static int fullTableCheck(String stmt, int offset) {
        ...
        Matcher ma = pattern.matcher(stmt.replaceAll("`", ""));
        if (ma.matches()) {
            return FULL_TABKES;
        }
        return OTHER;
