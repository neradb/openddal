...
public class ReplaceExecutor extends ExecutionFramework {
    ...
    public int doUpdate() {
        if (workers != null) {
            ...
            while (rows.next()) {
                ...
                addReplaceRowFlushIfNeed(newRow);
            }
            flushReplaceRows();
            rows.close();
            return affectRows;
    ...
    private void prepareReplace(TableMate table, List<Row> rows) {
    ...
    private synchronized void addReplaceRowFlushIfNeed(Row replace) {
        replaceRows.add(replace);
        if (replaceRows.size() >= QUERY_FLUSH_THRESHOLD) {
            flushReplaceRows();
        }
    }

    private synchronized void flushReplaceRows() {
        try {
            TableMate table = toTableMate(prepared.getTable());
            if (replaceRows.isEmpty()) {
                return;
            }
            prepareReplace(table, replaceRows);
    ...
    protected String doExplain() {
        ...
        if (workers != null) {
            ...
            explain.append("replace into ").append(table.getName()).append(" with query result");
